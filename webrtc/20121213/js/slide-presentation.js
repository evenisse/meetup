...
SlideManager.prototype.initEvents = function() {
  ...
  // window.onhashchange = this.parseHistory;
};
...
SlideManager.prototype.prev = function() {
  /* remove captions from captionatorjs if present */
  var cueNode = document.getElementsByClassName("captionator-cue-canvas")[0];
  if (cueNode) {
    cueNode.style.display = 'none';
  }
...
SlideManager.prototype.parseHistory = function() {
  var hash = window.location.hash.split("#page");
  this.current = 0;
  if (hash.length == 2) {
    this.current = parseInt(hash[1]) - 1;
  }
...
SlideManager.prototype.setHistory = function(index) {
  var path = window.location.pathname + '#page' + (index + 1);
